...
function trainThreads(Net, xTrain, yTrain, xTest, yTest, batchsize, testBatchsize, nEpochs, η, nOuterIterations, nInnerIterations, activation, outpath)
    ...
    ∇w = [zeros(dType, (nNeurons[n+1], nNeurons[n])) for n = 1:Net.nLayers]
	  ...
	  # Arrays for ADAM
    # M_w = [zeros(Float16, (Net.nNeurons[i+1], Net.nNeurons[i])) for i = 1:Net.nLayers]
	  # M_b = [zeros(Float16, (Net.nNeurons[i+1])) for i = 1:Net.nLayers]
	  # V_w = [zeros(Float16, (Net.nNeurons[i+1], Net.nNeurons[i])) for i = 1:Net.nLayers]
	  # V_b = [zeros(Float16, (Net.nNeurons[i+1])) for i = 1:Net.nLayers]
