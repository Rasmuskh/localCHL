...
function k_means!(ps, numIter)
    xTrain, yTrain, xTest, yTest = loadData("MNIST", 1000, 0)
    init_template!(ps)
...
function proto(in::Integer, out::Integer, σ=HS;
             initV = Flux.glorot_uniform, initr = 3.0f0, α = 1.0f0, γ=0.5f0)
    V = initV(in, out)
...
function rbf(in::Integer, out::Integer;
             initV = Flux.glorot_uniform, ϵ0 = 1.0f0)
    V = initV(in, out)
    ϵ = ϵ0*ones(Float32, out)
    return rbf(V, ϵ)
end
...
function (a::rbf)(x::AbstractArray)
    ...
    numIn = size(x)[1]
    numOut = size(a.V)[2]
    ϵ_reshaped = reshape(a.ϵ, (1,:,1))

    ### Surprisingly this works: Try starting with ϵ low and then forcing it to grow through some regularization term!
    ###d = sqrt.(sum(abs2, a.V.-reshape(x, (numIn, 1, batchsize)), dims=1))
    ###z = 1 .- σ.(reshape(0.5*d, (numOut, batchsize)))

    dist_squared = (sum(abs2, a.V.-reshape(x, (numIn, 1, batchsize)), dims=1))
    z = reshape(exp.(-ϵ_reshaped.*dist_squared), (numOut, batchsize))
    return z
end
