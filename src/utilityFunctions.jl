...
function init_network(nNeurons, highLim)
	  ...
	  # w = [sqrt(2 / nNeurons[i]) * randn(rng, Float16, (nNeurons[i+1], nNeurons[i])) for i = 1:nLayers]
    w = [1/sqrt(nNeurons[i] * nNeurons[i+1]) * randn(rng, Float16, (nNeurons[i+1], nNeurons[i])) for i = 1:nLayers]
	  b = [zeros(Float16, (nNeurons[i+1])) for i = 1:nLayers]
...
function Clamp(z, low::dType=0.0, high::Float16=Inf)
